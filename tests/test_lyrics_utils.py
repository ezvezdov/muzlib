"""Unit tests for the lyrics retrieval and embedding utilities."""
...
class TestConvertToTimestamp:
    ...
    def test_standard_formatting(self, ms, expected):
        """Test standard conversions and base cases."""
    ...
    def test_rollover_edge_cases(self, ms, expected):
        """Test edge cases where milliseconds roll over into seconds or minutes."""
        assert _convert_to_timestamp(ms) == expected

    @pytest.mark.parametrize(
        "ms, expected",
        [
            # Larger times and type handling
            (3600000, "60:00.000"),   # Exactly one hour
            (3661001, "61:01.001"),   # One hour, one min, one sec, one ms
            (3661001.6, "61:01.001"), # Float input (should be converted to int)
        ],
    )
    def test_extended_durations_and_types(self, ms, expected):
        """Test larger time values (hours) and floating-point inputs."""
        assert _convert_to_timestamp(ms) == expected
...
class TestGetLyrics:
    ...
    def test_step_1_ytm_synced_lyrics(self, mock_get_ytm, mock_synced_search, ytmusic_mock):
    ...
    @patch("muzlib.lyrics_utils.syncedlyrics.search")
    def test_step_2_external_synced_lyrics(self, mock_synced_search):
    ...
    @patch("muzlib.lyrics_utils.syncedlyrics.search")
    @patch("muzlib.lyrics_utils.get_lyrics_ytm")
    def test_step_3_ytm_plain_lyrics(self, mock_get_ytm, mock_synced_search, ytmusic_mock):
    ...
    @patch("muzlib.lyrics_utils.syncedlyrics.search")
    def test_step_4_external_plain_lyrics(self, mock_synced_search):
    ...
    @patch("muzlib.lyrics_utils.syncedlyrics.search")
    def test_step_5_no_lyrics_found(self, mock_synced_search):
